...
Page({
  ...
  onEdit() {
    wx.navigateTo({ url: `/pages/task-edit/task-edit?id=${this.data.id}` });
  },
  ...
  onCheckin() {
    ...
    wx.navigateTo({ url: `/pages/checkin/checkin?taskId=${this.data.id}` });
  },
      ...
      success: (r) => {
        ...
        lp.taskCopy(this.data.id)
          .then((res) => {
            ...
            setTimeout(() => wx.redirectTo({ url: `/pages/task-detail/task-detail?id=${res.task_id}` }), 600);
          })
