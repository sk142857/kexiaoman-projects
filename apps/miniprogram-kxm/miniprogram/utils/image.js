/**
 * 课小满 - 图片工具（压缩 + base64，经 /api/lp/upload 上传）
 *
...
/** 压缩图片（wx.compressImage，quality 40 左右兼顾清晰度与体积） */
function compressImage(src, quality = 40) {
  return new Promise((resolve, reject) => {
    wx.compressImage({
      src,
      quality,
      success: (res) => resolve(res.tempFilePath),
      fail: () => resolve(src), // 压缩失败回退原图
    });
  });
}
      ...
      success: async (res) => {
        ...
        try {
          ...
          for (const it of items) {
            const compressed = await compressImage(it.tempFilePath);
...
module.exports = { compressImage, fileToBase64, chooseAndUploadImages, fileUrl, relPath, STORAGE_DOMAIN };
