...
- **家长一键切到孩子身份（各有手机也能用）**：家长/管理员在【孩子档案】点「以孩子身份进入」（`POST /api/lp/switchChild`）。
  **家谱即授权、运行时推导**：不再把「家长挂孩子」物化成绑定行——家长名下孩子由其绑定的主家长账号经 `lp_children` 推导为“可用身份”，
  免码、免绑定即可切入，签发孩子身份 token 后以孩子身份操作（打卡/完成任务等）。
- **两条通道互不影响（核心场景）**：
  - 孩子自己的手机：凭**学生邀请码**显式绑定（`source=invite`），独立登录/独立会话；
  - 家长的手机：凭「我是孩子的家长」自动获得该孩子身份（`source` 无、运行时推导），无需消耗学生码；
  - 两者共用同一孩子账号的数据（孩子只有一份数据），但各持独立 token、随时进出、互不登出；作废孩子自己的绑定不影响家长切孩，反之亦然。
- **共用微信（家长 + 孩子一台手机）**：同一 openid 可同时显式绑定家长与孩子身份，或家长（显式）带孩子（推导）并存，
  「我的」页/身份切换可切换；切到家长身份若已设 PIN 锁则需校验 PIN（防孩子越权切家长模式）。

**孩子档案**：主家长在【我的】→【孩子档案】维护（姓名/性别/出生年月/学校/年级班级如 四（6），班级 1-35），并为每个孩子生成学生邀请码。
**绑定关系（含家谱继承）**：后台「成员管理 → 绑定管理」展示 `t_lp_students`（openid ↔ 账号）与 `source` 来源：
  - `register` 注册建档：家长/个人自建账号即绑定自己的手机；
  - `invite` 邀请码绑定：孩子自己的手机、家属、换绑等凭码进入；
  - `auto` 家长自动挂接：**仅存量历史行**（旧版把“家长切孩”物化成绑定）。新版起不再新增 `auto` 行——家长切孩改由「家谱继承」在运行时推导，
    因此 `auto` 行仅作历史展示/可安全删除（删除后家长仍可即时切孩）。
  - 说明：同一孩子账号可同时存在 `invite`（孩子自己的手机）与 `auto` 历史行（家长的手机）等，彼此独立、互不影响；
    身份“可用列表”= 显式绑定 ∪ 名下孩子（去重），见 `lpAuth.listBoundStaffs / openidMayUseStaff`。
...
- **删除收敛（减法）**：普通「删除」仅保留给后台 admin 账号；家长/孩子/家属/个人等业务身份一律走「物理清除」（带预览+完整审计），避免普通级联残留孩子/关联形成脏数据。
...
「成员管理 → 数据清理」（`/module/data_clean`，`shared/backend/dataCleanLib.js`）：与「物理清除/物理清理」互补——物理清除是按“账号/家庭/用户”整棵删（带媒体+审计），本模块用于**历史遗留孤儿/残留关联的定点清理**。

- **类目**（进入自动统计、逐类确认后执行，结果写「操作审计」）：
  - 低风险：已作废邀请码、存量 `source=auto` 绑定行、绑定/邀请码指向已删账号、孤儿孩子档案、孤儿家属关系、空壳孤儿学生账号（无档案/绑定/任何业务数据）；
  - 谨慎：空壳家长/家属/个人业务账号（可能含“已建档未扫码”账号，默认不纳入一键清理）。
- **安全约束**：仅删“确定无用”记录；绝不触碰 admin 与受保护超管；不做猜测性删除（如 openid 画像缺失的绑定）。
- 有业务数据的主家长/孩子请使用「物理清除」，勿用本模块。
- 接口：`GET /api/data_clean/preview`（计数）、`POST /api/data_clean/run`（执行指定类目）。
- **部署**：执行 `sql/upgrade_054_data_clean_menu.sql`（菜单 51），再重新部署云托管与后台前端。
- **菜单减负**：后台前端菜单为两层结构（分组→叶子），成员管理按“关系类 → 账号/清理类”排序平铺展示（关系类在前：家庭关系/孩子档案/家属关系/绑定管理/邀请码管理；账号/清理类在后：用户管理/注销管理/数据清理/物理清除审计），删除/清理入口与关系维护相邻可辨。
